...
;(function($){
    ...
    //调整元素窗口 比如 浏览器窗口改变  id是sss的元素溢出  那么 $('#sss').windowSize();
    $.fn.windowSize=function(size){
        !size&&(size=0);
        var t=$(this);
        var p= t.parents('.scroll')||$(window);
        t.css('max-height',function(){
            var h= p.height()+size;
            h=h>500?500:h;
            return h;
        });
    }
    //form序列对象  把form表单的内容 自动变成  {name:value}的格式 可直接传给后端
    ...
    $.fn.tabToArr=function(){
        var o = {};
        $.each(this, function() {
            if(!$(this).attr('data-name')){return;}
            o[$(this).attr('data-name')] = $(this).text()||'';
        });
        return o;
    };
    $.fn.tabToObj=function(){
        var o = [],q={};
        $.each(this, function() {
            var p={},v=$(this).attr('data-name');
            if(v=='工资号'){q.wageNo=$(this).text()}
            if(v=='备注'){q.remark=$(this).text()}
            if(!v||v=='职工号'||v=='工资号'||v=='姓名'||v=='备注'){return;}
    ...
    $.fn.objectToArray=function(key,val){
        var arr = [];
        $.each(this, function() {
    ...
    $.fn.getSequence=function(pageNo,pageSize){
        var n=1+((pageNo-1)*pageSize);
        this.find("tr").each(function(){
            var t=$(this),i= t.index()+n;
            t.find('td:eq(0)').text(i);
        });
    }
    $.fn.changeCheck=function(){
        this.each(function(){
    ...
    $.fn.isChecked=function(){
        var t=true;
        this.each(function(){
            if(!$(this).parent('.checkbox-custom').hasClass('checked')){
                return t=false;
            }
        });
        return t;
    }
    //obj{leftSelector:'左边选择器',rightSelector:'右边选择器',colClass:'右边列宽Class'}
    $.fn.leftToggle=function(toggleBtn,obj){
        var colClass=obj.colClass;
        var left=obj.leftSelector;
        var right=obj.rightSelector;
        function leftToggle(){
            if($(rightContent).hasClass(obj.colClass)){
                leftHide();
            }else{
                leftShow();
            }
        ...
        function leftHide(){
            $(left).addClass('hide');
            $(right).removeClass(colClass).addClass('col-sm-12');
            $(toggleBtn).children().removeClass('icon-double-angle-left').addClass('icon-double-angle-right');
        };
        function leftShow(){
            $(left).removeClass('hide');
            $(right).removeClass('col-sm-12').addClass(colClass);
            $(toggleBtn).children().removeClass('icon-double-angle-right').addClass('icon-double-angle-left');
        }
        leftToggle();
        return {
            show:function(){
    ...
    /**
     * [leftToggle resizeScreenFull]
     * @param  {[默认选项]}   options      [{leftSelector:'左边选择器',rightSelector:'右边选择器',colClass:'右边列宽Class'}]
     * @param  {Function} callback [回调全屏状态]
     * @return {[self]}            [this]
     */
    $.fn.resizeScreen=function(callback,options){
        var isFull=false;
        var Default={leftSelector:'#leftContent',rightSelector:'#rightContent',colClass:'col-sm-8',fullClass:'icon-resize-full',smallClass:'icon-resize-small'};
        ...
        function leftToggle(){
            if($(right).hasClass(obj.colClass)){
                leftHide();
            }else{
                leftShow();
            }
        ...
        function leftHide(){
            ...
            $(left).addClass('hide');
            $(right).removeClass(colClass).addClass('col-sm-12');
            icon.removeClass(obj.fullClass).addClass(obj.smallClass);
        ...
        function leftShow(){
            ...
            $(left).removeClass('hide');
            $(right).removeClass('col-sm-12').addClass(colClass);
            icon.removeClass(obj.smallClass).addClass(obj.fullClass);
            $('.scroll').mCustomScrollbar('update');
        }
        return {
            show:function(){
    ...
    //获取标签所有子节点的的属性值
    $.fn.getPropertyByAttr=function(key,value){
        key=key?key:"data-name";
        value=value?value:"data-value";
        var o = {};
        $.each(this, function() {
            if(!$(this).attr(key)){return;}
            o[$(this).attr(key)] = $(this).attr(value) || '';
        });
        return o;
    }
    //设置表单
    $.fn.setformByAttrKey = function (jsonValue,key) {
        key=key?key:"name";
        var obj=this,o=null;
        $.each(jsonValue, function (id, ival) {
                o=obj.find('['+key+'="'+id+'"]');
                if(o.attr('type')=='radio'){
                    obj.find('['+key+'="'+id+'"]'+'[value="'+ival+'"]').radio('check');
                }else if(o.attr('type')=='checkbox'){
                    if(typeof ival =='object'){
                        $.each(ival,function(){
                            obj.find('['+key+'="'+id+'"]'+'[value="'+this+'"]').checkbox('check');
                        });
                    }else{
                        obj.find('['+key+'="'+id+'"]'+'[value="'+ival+'"]').checkbox('check');
                    }

                }else if(o.hasClass('selectpicker')){
                    obj.find('['+key+'="'+id+'"]').selectpicker('val', ival);
                }else{
                    obj.find('['+key+'="'+id+'"]').val(ival);
                }
            }
        )
    };

    $.fn.getExamTemplate=function(courseList,listOptions){

        var titleTp=$('thead>tr').empty().append("<th>#</th>");
        //模板头
        var tpHeader='<% var index=1;%><%_.each(result,function(item,key){%><tr><td data-name="index" data-value="<%=index%>"><%=index%></td>'
        //模板正文
        var tableBodyTp='';
        $.each(courseList,function(index, item) {
            var courseName=$(item).attr(name);
            tableBodyTp+='<td data-name="'+$(item).attr('name')+'" data-value="'+'<%=item.'+$(item).attr('name')+'%>"><%=item.'+$(item).attr('name')+'%></td>';
            titleTp.append('<th>'+$(item).find('span').text()+'</th>');
        });
        $.each(listOptions,function(index, item) {
            tableBodyTp+='<td data-name="'+$(item).attr('name')+'" data-value="'+'<%=item.'+$(item).attr('name')+'%>"><%=item.'+$(item).attr('name')+'%></td>';
            titleTp.append('<th>'+$(item).find('span').text()+'</th>');
        });
        //模板脚
        var tpFooter='</tr><%=index++%><%});%>';
        var template=tpHeader+tableBodyTp+tpFooter;
        return template;
    };
...
(function (factory) {
    ...
    $.fn.setTable = function(){
        ...
        function init(){
            table.find('tr').css('white-space', 'nowrap');
            $('.scroll').mCustomScrollbar('update');
        };
        init();

    };
    $.fn.resetScrollbarSetting = function(){
        $(this).mCustomScrollbar('destroy');
        $(this).mCustomScrollbar({axis:'yx',mouseWheel:{enable: true, scrollAmount:'800px', axis: 'y'} ,advanced: {autoScrollOnFocus: false}});
        $('.scroll').mCustomScrollbar('update');
    };
    /* 插入cookies到考试
     * cookie 名 item_exam
     * @param obj_insert 保存的数据 如果obj_insert为空则使用 serializeObject() 获得的数据
     * @callback $(this)
     */
    $.fn.insertExamCookies = function(obj_insert){
        var defaults = {};
        var $this = $(this);
        obj_insert = obj_insert?((typeof obj_insert == 'object')?obj_insert: eval('('+obj_insert+')')):false;
        // if (!obj_insert) {
        // 	return false
        // }
        defaults = eval('('+$.cookie('item_exam')+')');
        return this.each(function(){
            var insert_cookies = {};
            var o = $this.serializeObject();
            if (o == null) {
                insert_cookies = $.extend(defaults, obj_insert);
            } else {
                insert_cookies = $.extend(defaults, o);
            }
            $.cookie('item_exam', (typeof insert_cookies == 'object')?JSON.stringify(insert_cookies): insert_cookies);
            return $(this);
        });
    };
    /* 查询考试cookies
     * cookie 名 item_exam
     * @param option
     * @callback 返回考试cookies对象
     */
    $.fn.getExamCookies = function(options){
        var defaults = {
            action: 1
        };
        var opts_exam = $.extend(defaults, options);
        var obj_get_exam = eval('('+$.cookie('item_exam')+')');
        if (!opts_exam.action) {
            $(this).setform(obj_get_exam);
            $('.selectpicker').selectpicker('refresh');
        };
        return obj_get_exam;
    }
    /*
     * 获取元素所有的属性 @param null @callback 所有属性的obj
     */
    $.fn.getAllAttrs = function(){
        $this = $(this);
        var obj = {};
        $.each($this[0].attributes, function(index, el) {
            obj[el.localName] = el.value;
        });
        return obj;
    }
    /* 表格列排序
     * example  $('table').sortTableByCol([{id: 1},{id: 2}], function(handle){})
     * @param data 表格头的属性对象
     * @callback 返回表格对象
     */
    $.fn.sortTableByCol = function(data, callback){
        $this = $(this);
        data = data?data:{};
        function init_thead(data_attr){
            $.each(data_attr, function(key, item){
                $.each(item, function(key_key, item_item){
                    $this.find('thead>tr>th').not('th:first').eq(item_item-1).attr(key_key, item_item);
                });
            });
        };
        init_thead(data);
        return this.each(function(){
            ...
            $.each(thead_tr, function(index, item){
                $(item).find('th').not('th:first').bind('click', function(event) {
                    $(this).siblings('th').find('span').remove();
                    if ($(this).find('span:first').hasClass('icon-chevron-down')) {
                        $(this).find('span:first').remove();
                        $(this).append('<span class=" icon-chevron-up pull-right" style="font-size:20px"></span>');
                        callback($(this), $(this).getAllAttrs());
                        return $(this);
                    }
                    $(this).find('span:first').remove();
                    $(this).append('<span class="icon-chevron-down pull-right" style="font-size:20px"></span>');
                    callback($(this), $(this).getAllAttrs());
                }).css('cursor', 'pointer');
            });
        });
